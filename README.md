...
| servidor MCP | `REQUEST_STATE_TTL_S` | `600` | Validade do `requestState` em segundos: aceita **somente 300 a 1800** (5 a 30 min, faixa do enunciado); fora disso o boot falha (exit 2). Ver "Decisões técnicas". |
| servidor MCP | `REQUEST_STATE_TTL_S_SOMENTE_TESTE` | (não definida) | **Só para os testes** de expiração (1 a 1800 s); tem precedência sobre `REQUEST_STATE_TTL_S` e o boot a sinaliza (`ttl_de_teste: true` + aviso em stderr). Não use fora dos testes. |
...
**Proteção do `requestState`.** `servidor-mcp/app/server.py:criar_servidor` monta o `MCPServer` com `RequestStateSecurity(keys=[REQUEST_STATE_SECRET], ttl=600)`, do SDK `mcp` 2.2.0 (`mcp/server/request_state.py`): o estado é cifrado e autenticado com **AES-256-GCM**, com a chave derivada por **HKDF-SHA256** do segredo (detalhe abaixo). O selo é vinculado ao **método**, ao **nome da tool** e ao **digest dos argumentos**, então mexer nos argumentos do retry invalida o estado. Adulterado, expirado ou de outra chave, o estado é rejeitado com **`-32602` "Invalid or expired requestState"** (HTTP 400, nunca 500). Sem `keys=`, o SDK usaria uma **chave process-local aleatória**: o estado não sobreviveria a um restart e `REQUEST_STATE_SECRET` seria ignorado sem aviso. Por isso a configuração é sempre explícita, o boot falha sem o segredo (ausente, placeholder, não hexadecimal, menor que 32 bytes ou sem aleatoriedade: menos de 16 bytes distintos, padrão repetido de até 8 bytes ou progressão aritmética) e há teste de restart do servidor. O segredo só entra por variável de ambiente e nunca é impresso, nem o valor nem o tamanho.

**Como o segredo vira chave (o que o SDK realmente faz).** `REQUEST_STATE_SECRET` (64 caracteres hexadecimais = 32 bytes aleatórios) é entregue ao SDK como **texto**: em `mcp/server/request_state.py:205` (`AESGCMRequestStateCodec.__init__`) cada chave `str` vira bytes com `k.encode()` (UTF-8) e só se exige `len(k) >= 32`. A derivação está em `request_state.py:182`: `_derive_key` ("Stretch an operator secret (>= 32 bytes, any format) into the AES-256 key") faz `HKDF(algorithm=SHA256(), length=32, salt=None, info=_KDF_INFO).derive(secret)`. Ou seja, a chave AES-256 **não é o hexadecimal decodificado**: é o HKDF-SHA256 dos 64 bytes ASCII do texto. A entropia continua sendo a dos 32 bytes aleatórios que o operador gerou; o hex só é a forma de transportá-los.

**Checagem de entropia do segredo (política local, além do enunciado).** O enunciado só pede que o segredo venha do ambiente. A recusa de padrões triviais (menos de 16 bytes distintos, padrão repetido de até 8 bytes, progressão aritmética) é uma **política local** deste projeto, uma barreira contra descuido e não contra um operador malicioso. Um `secrets.token_hex(32)` real não é rejeitado: há um teste que sorteia 10 000 segredos com `token_hex(32)` e exige que todos passem (`servidor-mcp/tests/test_correcoes_entrada.py:test_token_hex_real_nunca_e_recusado`).

**Por quanto tempo vale.** 600 s (10 min, dentro da faixa de 5 a 30 min do enunciado). `REQUEST_STATE_TTL_S` aceita **somente 300 a 1800 s** (fora da faixa o boot falha com exit 2). Os testes de expiração precisam de um TTL de segundos e usam uma variável à parte, `REQUEST_STATE_TTL_S_SOMENTE_TESTE` (1 a 1800 s): ela tem precedência, existe só para os testes, **não é a via de produção** e, quando usada, o boot registra `ttl_de_teste: true` e um aviso em stderr (sem ela, o boot registra o TTL efetivo). O estado é reutilizável dentro da validade: **não há nonce**, e o servidor não guarda estado nenhum; repetir um estado antigo reexecuta a revalidação (se a sala já foi reservada, o servidor pergunta de novo em vez de reservar em duplicidade). O `tools/call` **nunca é repetido pelo host** por falha de transporte (só leituras são refeitas uma vez), para não duplicar reserva.
...
**Falha de transporte na continuação (a pausa é retomável enquanto o `requestState` valer).** Se o MCP estiver fora do ar (conexão recusada ou timeout) quando o usuário envia `escolha=<v>`, a Task **permanece** `TASK_STATE_INPUT_REQUIRED`, o `PausedState` fica **intacto** e a mensagem é `Servidor MCP indisponivel; reenvie escolha=<v>`. O usuário reenvia quando o servidor voltar, e a retomada funciona **enquanto o `requestState` continuar válido no servidor** (600 s por padrão, contados a partir da última rodada). Depois de expirado, a continuação falha com o erro `-32602` "Invalid or expired requestState" do servidor (a Task vai a `TASK_STATE_FAILED`) e é preciso **refazer o pedido**: a pausa preservada no agente não devolve um estado que o servidor já não aceita. O retry de `tools/call` pode até ter chegado ao servidor; ele revalida a cada rodada e não duplica a reserva. Só terminam em `TASK_STATE_FAILED` o erro de protocolo (`requestState` inválido/expirado, `-32602`) e o `isError` da tool. Na primeira chamada (sem pausa) uma falha de transporte continua terminando a Task em `TASK_STATE_FAILED` com erro claro.
...
**Parser do pedido (só traduz).** O agente não valida regra de domínio. `escolha=<v>` aceita espaços nas pontas (`escolha= sala-fusca` vale `sala-fusca`), mas o valor tem que estar nas opções recebidas. Aspas em `responsavel` não são especiais: `responsavel="Doc Brown"` grava as aspas como parte do nome. `inicio` e `fim` seguem **verbatim** para o servidor: o agente não valida instante, fuso nem forma de data, e o servidor é o único dono dessa validação. Um ISO sem fuso ou um texto qualquer termina em `TASK_STATE_FAILED` com a mensagem do servidor, "Formato invalido: inicio e fim devem ser ISO 8601 com fuso" (erro de execução da tool, `isError`). O agente só recusa o que é estrutura do pedido: chave ausente, repetida ou desconhecida, valor vazio, `responsavel` acima de 200 caracteres, quebra de linha embutida.

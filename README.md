...
**Parser do pedido (só traduz).** O agente não valida regra de domínio. `escolha=<v>` aceita espaços nas pontas (`escolha= sala-fusca` vale `sala-fusca`), mas o valor tem que estar nas opções recebidas. Aspas em `responsavel` não são especiais: `responsavel="Doc Brown"` grava as aspas como parte do nome. O `REQUEST_STATE_TTL_S` mínimo de 1 s existe só para testar expiração.

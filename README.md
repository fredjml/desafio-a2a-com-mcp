...
**Parser do pedido (só traduz).** O agente não valida regra de domínio. `escolha=<v>` aceita espaços nas pontas (`escolha= sala-fusca` vale `sala-fusca`), mas o valor tem que estar nas opções recebidas. Aspas em `responsavel` não são especiais: `responsavel="Doc Brown"` grava as aspas como parte do nome. O TTL de 1 s só é possível pela variável de teste `REQUEST_STATE_TTL_S_SOMENTE_TESTE`.
